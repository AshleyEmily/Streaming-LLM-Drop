...
class DeepseekConfig(PretrainedConfig):
    ...
    def _rope_scaling_validation(self):
        ...
        Updated for v2: accepts all modern RoPE scaling types including llama3, yarn, longrope, etc.
        """
        ...
        # Accept both "type" (old) and "rope_type" (new transformers key)
        rope_scaling_type = self.rope_scaling.get("type") or self.rope_scaling.get("rope_type")
        known_types = ["linear", "dynamic", "llama3", "yarn", "longrope", "su", "su_scaled"]
        if rope_scaling_type is not None and rope_scaling_type not in known_types:
            import warnings
            warnings.warn(
                f"`rope_scaling` type '{rope_scaling_type}' is not in the known list {known_types}. "
                "Proceeding anyway — this may be a newer RoPE variant."
            )